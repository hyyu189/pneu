...
> Status: historical record — M3 acceptance round
...
**Tree note.** HEAD moved to `1815b34` while I was reviewing. `git diff 570a940..HEAD --stat` is `handoff/archive/design-v1-central-mail.md` + `handoff/upstream-codex-zero-turn-resume.md`, 2 files, docs only, and the change is entirely §8 (Codex preallocation deferred). §7 is untouched. Every product-code finding below was measured against code byte-identical to `570a940`.
