...
| Grok Build `0.2.118` | Isolated stdlib ACP supervisor; fenced lease and identity checks; bounded HOME/XDG/GROK_HOME/TMP/log state; exact mail-only permission policy; child-death recovery; focused fault, mutation, soak, and three-seat interop coverage | Fresh valid credentialed two-generation adapter E2E and extracted release-artifact install/smoke; the current host OAuth credential returned ACP 403 `unauthenticated:bad-credentials`, and no refresh was attempted |
...
Grok/Claude/Codex mail interop. The previous isolated Stage 2 lab proved one
valid ACP mail-to-wake-to-drain/ack generation. The 2026-08-06 product-adapter
credentialed rerun was attempted read-only but the host OAuth credential had
expired and ACP returned 403 `unauthenticated:bad-credentials`; the adapter
correctly failed closed without acknowledging the message. Grok therefore
remains an implementation/test claim, not a promoted public support claim.
